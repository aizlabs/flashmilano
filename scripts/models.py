...
class AudioConfig(BaseModel):
    ...
    public_base_url: Optional[str] = Field(
        default=None,
        description="Public CDN base URL",
    )
...
class LanguageConfig(BaseModel):
    ...
    target_language: str = Field(default=DEFAULT_TARGET_LANGUAGE, description="Language generated by the pipeline")
    target_language_code: str = Field(default=DEFAULT_TARGET_LANGUAGE_CODE, description="Short language code")
    locale: str = Field(default=DEFAULT_LOCALE, description="Locale for language-specific formatting")
    learner_native_language: str = Field(
        default=DEFAULT_LEARNER_NATIVE_LANGUAGE,
        description="Native language used for learner-facing translations",
    )
    spacy_model: str = Field(default=DEFAULT_SPACY_MODEL, description="SpaCy model for NLP steps")
    glossary_heading: str = Field(default=DEFAULT_GLOSSARY_HEADING, description="Visible glossary section heading")
    legacy_glossary_headings: List[str] = Field(
        default_factory=lambda: list(DEFAULT_LEGACY_GLOSSARY_HEADINGS),
        description="Accepted headings when parsing older generated posts",
    )
    prompt_pack: str = Field(default=DEFAULT_PROMPT_PACK, description="Prompt pack identifier")
    glossary_rules: str = Field(default=DEFAULT_GLOSSARY_RULES, description="Glossary rules profile identifier")
    site_name: str = Field(default=DEFAULT_SITE_NAME, description="Public site/brand name")
